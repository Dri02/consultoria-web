  ...
  export default function Home() {
    ...
    const deleteConsultancy = async () => {
      ...
      try {
        ...
        const response = await axios.post("http://localhost:3002/deleteFileW", payload, {
          headers: { "Content-Type": "application/json" },
        });
    
        // 2. Actualizar estados locales inmediatamente (optimista)
        setFolders(prev => prev.filter(name => name !== folderName));
        setFolderData(prev => prev.filter(item => item.name !== folderName));

        // if (response.data.reload) {
        //   window.location.reload(); // Recarga la página
        // }
    
        // 3. Forzar recarga completa desde el servidor
        await getFoldersData(); 
    
        // 4. Resetear estados relacionados
        setShowOptions(false);
        setIsUpdateFolderData(true);
        setDeleted(true)
        navigate("/home")
        ...
        // Revertir cambios si falla la eliminación en servidor
        getFoldersData();
      }
    };
    ...
    useEffect(() => {
      if (deleted) {
        const reloadData = async () => {
          await getFoldersData();
          setIsUpdateFolderData(false); // Resetear la bandera
        };
        reloadData();
        setTimeout(() => {
      ...
      setDeleted(false);
    }, [isUpdateFolderData]); // Escuchar cambios en esta bandera
